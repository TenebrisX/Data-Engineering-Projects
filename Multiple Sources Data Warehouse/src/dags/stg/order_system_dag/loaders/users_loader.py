...
class UsersLoader(MongoLoader):
    """
    Concrete class for loading user data into PostgreSQL from MongoDB.
    """
    ...
    def _get_or_create_workflow_setting(self, conn: PgConnect) -> EtlSetting:
        """
        Get or create the workflow setting for user data loading.

        Args:
            conn (PgConnect): PostgreSQL connection.

        Returns:
            EtlSetting: Workflow setting for user data loading.
        """
    ...
    def _process_and_save_documents(self, conn: PgConnect, load_queue: list) -> None:
        """
        Process and save user documents to PostgreSQL.

        Args:
            conn (PgConnect): PostgreSQL connection.
            load_queue (list): List of user documents from MongoDB.
        """
        i = 0
        for d in load_queue:
    ...
    def _update_workflow_setting(self, conn: PgConnect, wf_setting: EtlSetting, load_queue: list) -> None:
        """
        Update the workflow setting after user data loading.

        Args:
            conn (PgConnect): PostgreSQL connection.
            wf_setting (EtlSetting): Workflow setting for user data loading.
            load_queue (list): List of user documents from MongoDB.
        """
